...
Flow:
  1. Fetch items from Clover
  2. Transform (anonymize, filter fields)
  3. Upsert into SQLite
  4. Fetch orders for the target date range
  5. For each order, fetch line items
  6. Aggregate line items to daily totals in memory
  7. Upsert daily_sales into SQLite
  8. Fetch stock snapshots
  9. Insert stock_snapshots into SQLite
  10. Log sync result
"""
...
from app.etl.transform import clean_item, clean_stock, aggregate_line_items
